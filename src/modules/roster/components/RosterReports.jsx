      ...
      <div className="flex-1 min-h-0 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar relative">
          <table className="w-full text-left text-sm whitespace-nowrap border-collapse min-w-max">
            ...
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-200 shadow-[0_1px_0_0_#e2e8f0] sticky top-0 z-30">
              <tr>
                <th className="px-6 py-4 font-bold uppercase tracking-wider sticky left-0 top-0 bg-slate-50 z-40 w-56 border-r border-slate-200 shadow-[1px_0_0_0_#e2e8f0]">
                  Employee Record
                </th>
                
                <th className="px-6 py-4 font-bold uppercase tracking-wider border-r border-slate-100">
                  Timeline Overview
                </th>
                
                <th className="px-4 py-4 font-bold uppercase tracking-wider text-center border-r border-slate-100 bg-emerald-50/50 text-emerald-700">
                  Total Worked
                </th>

                {shifts.map(shift => (
                   <th key={shift.id} className="px-4 py-4 font-bold uppercase tracking-wider text-center border-r border-slate-100">
                ...
                <th className="px-4 py-4 font-bold uppercase tracking-wider text-center border-r border-slate-100 text-rose-600 bg-rose-50/50">
                  Total Offs
                </th>
                
                <th className="px-4 py-4 font-bold uppercase tracking-wider text-center text-amber-600 bg-amber-50/50">
                  Leaves
                </th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {reportData.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-6 py-3 font-bold text-slate-800 flex items-center gap-4 sticky left-0 bg-white z-20 w-56 border-r border-slate-100 shadow-[1px_0_0_0_#f1f5f9] group-hover:bg-slate-50/80">
                      <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center text-xs font-black shrink-0 border border-blue-100 shadow-sm">
                          {row.name.charAt(0)}
                      </div>
                  ...
                  <td className="px-6 py-2 border-r border-slate-100">
                      <div className="flex gap-4 items-center w-max py-1">
                          {row.weeks.map((week, wIdx) => (
                              <div key={wIdx} className="flex gap-1.5 p-1.5 bg-slate-50/60 rounded-xl border border-slate-100 shadow-inner">
                                  {week.map(status => {
                                      let cellClasses = '';
                                      let badgeClasses = '';
                                      ...
                                      if (status.status === 'worked') {
                                          ...
                                          cellClasses = 'bg-slate-50 border-slate-200 text-slate-400';
                                          ...
                                          >
                                              ...
                                              <div className="flex justify-center w-full mt-auto">
                                                  {status.status === 'worked' ? (
                                                      <span className={`text-[8px] font-extrabold uppercase tracking-wider px-1 py-[2px] rounded w-full text-center truncate ${badgeClasses}`}>
                                                          {status.shiftCode}
                                                      </span>
                                                  ) : (
                  ...
                  <td className="px-4 py-3 text-center font-black text-emerald-700 bg-emerald-50/30 text-base border-r border-slate-100">
                     {row.daysWorked}
                  </td>
                  
                  {shifts.map(shift => (
                    <td key={shift.id} className="px-4 py-3 text-center font-bold text-slate-600 text-sm border-r border-slate-100">
                       {row.shiftCounts[shift.id] > 0 ? (
                           <span className={`px-2.5 py-1 rounded-md ${shift.color} bg-opacity-30 shadow-sm border`}>
