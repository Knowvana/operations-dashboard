...
import { Activity, BarChart2, List, Layers, Upload, Database } from 'lucide-react'; 
...
import OpsShiftConfig from './components/OpsShiftConfig';
import OpsDataManagement from './components/OpsDataManagement';

export default function OpsMonitorApp({ user }) {
  // --- Module State ---
  ...
  const SETTINGS_TABS = [
    { id: 'shifts', label: 'Shift Config', icon: Layers },
    { id: 'import', label: 'Import Tasks', icon: Upload },
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-400 font-light">Loading Ops Data...</div>;

  return (
    <div className="flex-1 bg-gradient-to-br from-blue-50/50 via-teal-50/50 to-green-50/50 flex flex-col relative min-h-0">
      <Header 
          ...
          <>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex bg-white rounded-lg p-1 border border-slate-200 shadow-sm">
                ...
                <button onClick={() => setViewMode('tasks')} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${viewMode === 'tasks' ? 'bg-indigo-50 text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}><List size={14} /> Tasks</button>
              </div>
            </div>
      ...
      {/* Universal Settings Modal Popup */}
      <SettingsModal
      ...
      >
        {(activeModalTab) => (
          <>
            {activeModalTab === 'shifts' && (
              <OpsShiftConfig shifts={shifts} onSave={setShifts} onClose={() => setIsSettingsOpen(false)} />
            )}
            ...
            {activeModalTab === 'import' && (
              <div className="h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                <ImportTasksModal 
                  ...
                  onViewTasks={() => { setIsSettingsOpen(false); setViewMode('tasks'); }}
                  existingTasks={tasks}
                />
              </div>
            )}
            ...
            {activeModalTab === 'data' && (
              <OpsDataManagement onDataAction={handleDataAction} />
            )}

            {/* The confirmation dialog overlays exactly inside the modal container when triggered */}
            <ConfirmationModal
